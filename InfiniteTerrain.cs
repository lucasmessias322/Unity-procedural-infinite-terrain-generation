...
// public GlobalLayerDefinition globalLayerDefinition;
...
public class InfiniteTerrain : MonoBehaviour
{
    ...
    async void CriarChunkAsync(Vector2Int coord)
    {
        ...
        // Calcula quantas layers teremos: as do bioma + global (se definida)
        int biomeLayerCount = (biomeAtCenter.terrainLayerDefinitions != null ? biomeAtCenter.terrainLayerDefinitions.Length : 0);
        bool useGlobalLayer = (globalLayerDefinition != null && globalLayerDefinition.terrainLayer != null);
        int totalLayers = biomeLayerCount + (useGlobalLayer ? 1 : 0);
            ...
            if (totalLayers > 0)
            {
                ...
                for (int z = 0; z < alphamapResolution; z++)
                {
                    for (int x = 0; x < alphamapResolution; x++)
                    {
                        ...
                        // Calcula o peso para a camada global (apenas baseado na altura)
                        if (useGlobalLayer)
                        {
                            float globalWeight = (worldHeight >= globalLayerDefinition.minHeight && worldHeight <= globalLayerDefinition.maxHeight) ? 1f : 0f;
                            weights[totalLayers - 1] = globalWeight;
                            totalWeight += globalWeight;
                        }

                        if (totalWeight == 0f)
                        {
                            // Caso nenhum peso seja atribuído, define fallback para a primeira camada
        ...
        // Configura as layers do TerrainData usando as layers do bioma e a camada global, se definida
        if (totalLayers > 0)
        {
            ...
            if (useGlobalLayer)
            {
                layers[totalLayers - 1] = globalLayerDefinition.terrainLayer;
            }
            terrainData.terrainLayers = layers;
            terrainData.SetAlphamaps(0, 0, splatmapDataResult);
        }
